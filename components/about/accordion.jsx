            ...
            {dataO.map((e) => (
              <div className="flex flex-col text-xl gap-4 text-white/90">
                <div className="flex flex-col gap-2 custom-gradient-half p-2">
                  <span className="text-xl">{e.institution}</span>
                  <div className="flex gap-10 items-center">
                    <span className="text-lg">{e.deg}</span>
                    <span className="text-sm">{e.duration}</span>
                  </div>
                </div>
                <span className="tracking-wider text-lg">{e.details}</span>
              </div>
            ))}
          </motion.div>
        </motion.div>
        ...
        >
          {data.app.map((e) => (
            <motion.span
              ...
              className="px-6 py-2 bg-violet/20 backdrop-blur-2xl rounded-4xl mt-10 text-white/90"
            >
              {e}
            </motion.span>
          ))}
